...
export async function getRequestToken(payload) {
  ...
  return AXIOS.get(
    `${process.env.API_HOST}/${URL.GET_REQUEST_TOKEN}?api_key=${process.env.API_KEY}`
  )
    .then((response) => {
      return response;
    })
...
export async function getSessionId(payload) {
  ...
  const AXIOS = useAxios(header);
  return AXIOS.post(
    `${process.env.API_HOST}/${URL.LOGIN_PART2}?api_key=${process.env.API_KEY}`,
    payload
  )
    .then((response) => {
      return response;
...
export async function logIn(payload) {
  ...
  const AXIOS = useAxios(header);
  return AXIOS.post(
    `${process.env.API_HOST}/${URL.LOGIN_PART1}?api_key=${process.env.API_KEY}`,
    payload
  )
    .then(async (response) => {
