...
export async function getMovieDetail(payload) {
    ...
    const AXIOS = useAxios(header);
    return AXIOS.get(`${process.env.API_HOST}/${url}?api_key=${process.env.API_KEY}&language=en-US`)
        .then((response) => {
            return response;
        })
        .catch((error) => {
...
export async function listNowPlaying(payload) {
    ...
    const AXIOS = useAxios(header);
    return AXIOS.get(`${process.env.API_HOST}/${URL.NOW_PLAYING}?api_key=${process.env.API_KEY}&page=${payload.page}`)
        .then((response) => {
            return response;
        })
        .catch((error) => {
...
export async function listTopMovies(payload) {
    ...
    const AXIOS = useAxios(header);
    return AXIOS.get(`${process.env.API_HOST}/${URL.TOP_MOVIES}?api_key=${process.env.API_KEY}&page=${payload.page}`)
        .then((response) => {
            return response;
        })
        .catch((error) => {
